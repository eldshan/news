		...
		<view class="home-list">
			<view v-if="ishistory" class="label-box">
				...
				<view v-if="historyLists.length > 0" class="label-content">
					<view class="label-content__item" @click="openHistory(item)" v-for="(item,index) in historyLists" :key="index">
						{{item.name}}
					</view>
				</view>
			...
			<list-scroll v-else class="list-scroll">
				...
				<view v-if="searchList.length>0">
					<list-card @clickEvent="setHistory" :item="item" v-for="(item,index) in searchList" :key="item._id"></list-card>
				</view>
